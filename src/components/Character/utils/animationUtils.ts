...
const setAnimations = (gltf: GLTF) => {
  ...
  // Start playing keyboard / typing animations immediately
  if (gltf.animations) {
    ...
    // Pre-configure introAction so it's ready for startIntro()
  ...
  function startIntro() {
    ...
    // Guard: log and bail rather than crashing with a ! assertion
    if (!introClip) {
      console.error("introAnimation clip not found — cannot start intro.");
      return;
    }
    const introAction = mixer.clipAction(introClip);
    introAction.clampWhenFinished = true;
    introAction.stop().reset().fadeIn(0.2).play();
    setTimeout(() => {
